    ...
    #define BUILD_MESHX
	  ...
	  class HELPAPI MeshX : public DataX {
	  public:
		  MeshX ();
		  ~MeshX ();	
		  ...
		  xref		AddVert ( Vec3F vec )						{ return (this->*m_AddVertFunc) (vec.x, vec.y, vec.z); }
		  xref		AddVertNorm ( Vec3F vec );		
		  xref		AddVertClr ( Vec4F vec );
		  xref		AddVertTex ( Vec2F vec );		
		  ...
		  Vec3F*	GetVertPos ( int n )				{ return (Vec3F*)	GetElem(BVERTPOS, n);  }
		  Vec3F*	GetVertNorm( int n )				{ return (Vec3F*)	GetElem(BVERTNORM, n); }
		  Vec2F*	GetVertTex ( int n )				{ return (Vec2F*)	GetElem(BVERTTEX, n);  }
		  ...
		  void		SetVertPos ( int n, Vec3F pos )		{ SetElemVec3 ( BVERTPOS, n, pos) ; }
		  void		SetVertTex ( int n, float u, float v )	{ Vec2F x(u,v); SetElemVec2 ( BVERTTEX, n, x ); }

		  void		SetVertNorm ( int n, Vec3F norm )	{ SetElemVec3 ( BVERTNORM, n, norm); }
    ...
    #pragma message("  Warning: MeshX not built. Need DataPtr via _REQUIRE_DATAPTR.")
