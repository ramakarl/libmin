...
Vec3F Quaternion::operator* (const Vec3F& v) const
{
	...
	Vec3F uv, uuv;
	Vec3F qvec; qvec.Set(float(X), float(Y), float(Z));
	uv = qvec.Cross(v);
	uuv = qvec.Cross(uv);
	uv *= (2.0f * float(W));
	uuv *= 2.0f;
...
// sets new Quaternion based on euler angles
Quaternion& Quaternion::set(const Vec3F& vec)
{
...
Quaternion Quaternion::dual(Vec3F pos)
{
...
Quaternion& Quaternion::fromAngleAxis(f32 angle, const Vec3F& axis)
{
	angle *= 0.5f;
...
void Quaternion::toAngleAxis(f32& angle, Vec3F& axis) const
{
...
Vec3F Quaternion::rotateVec(Vec3F p)
{
	//--- efficient way		
	Vec3F u;
...
Quaternion& Quaternion::fromRotationFromTo(Vec3F from, Vec3F to, float frac)
{
	Vec3F axis = to;	
...
Quaternion& Quaternion::fromDirectionAndUp ( Vec3F fwd, Vec3F up )
{
	Vec3F side;
	fwd.Normalize();
...
Quaternion& Quaternion::fromDirectionAndRoll ( Vec3F fwd, float roll )
{
	Vec3F side, up;
	
	fwd.Normalize();
	side = fwd.Cross(Vec3F(0,1,0)); side.Normalize();
	up = side.Cross ( fwd ); up.Normalize();
	
	Matrix4F local;
	local.SRT ( side, up, fwd, Vec3F(0,0,0), 1.0 );
