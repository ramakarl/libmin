...
void Spline::AddChannel ( int ci, char eval, char dt )
{
	...
	// primary buffer
	ch.m_buf = AddBuffer ( b, "ch", sizeof(Vec4F), 0, DT_CPU );		

	// secondary buffer
	AddBuffer ( b+1, "chx", sizeof(Vec4F),	0 );			// stores: bspline temps, or catmull-rom midpoints, or bezier tangents
...
void Spline::SetKey (int ch, Vec4F val, int k)
{
	if ( k==-1 ) k = m_k;	
	int b = (ch+1)*2;
	SetElemVec4 ( b, k, val );	
	...
	if ( m_rebuild ) {
		...
		for (int c=0; c < m_Channels.size(); c++) {
			...
			case 'c': {											//---- Catmull-Rom preparation
				// construct midpoints				
				Vec4F* keyR = GetElemVec4 (b, 2 );		// inputs
				Vec4F* keyL = GetElemVec4 (b, 0 );
				Vec4F* mids = GetElemVec4 (b+1, 0);		// output
			...
			case 'z': {											//---- Bezier preparation
				...
				Vec4F* keyR = GetElemVec4 (b, 2);			// inputs
				Vec4F* keyL = GetElemVec4 (b, 0);				
				Vec4F* tans = GetElemVec4 (b+1, 0);			// output
...
{
	/*Vec3F* keyP = GetElemVec3 (SP_POS, 0);
	...
	for (int n = 0; n < m_num - 1; n++) {		
		...
		q.rotationFromTo(Vec3F(0, 1, 0), dir); q.normalize();
		*keyO = q;
		keyO++; keyP++;
	}*/
}
...
Vec4F Spline::EvaluateQSlerp ( int b, int k, float u )
{
...
// - Point
Vec4F Spline::EvaluatePoint ( int b, int k, float u )
{
	Vec4F* key = GetElemVec4 (b, k);
	return *key;	
}

// Scale interpolation
// - Linear vector interpolation
Vec4F Spline::EvaluateLinear ( int b, int k, float u )
{
	Vec4F* key = GetElemVec4 (b, k);
	if (k >= m_num) 
		return *key;
...
//
Vec4F Spline::EvaluateCatmullRom( int b, int k, float u )
{
	Vec4F* keys	= GetElemVec4 ( b, 0 );		// keys
	Vec4F* mids = GetElemVec4 ( b+1, 0 );		// midpoints
	...
	float h01 = -2*u3 + 3*u2;
	float h11 = u3 - u2;
	Vec3F kn1 = (k + 1 >= m_num) ? *(keys+m_num - 1) : *(keys+k + 1);
...
// p(t) = <Pj, Pj+1, Pj+2, Pj+3> Bmtx T u     where u = t - int(t),  P = input points, Bmtx = Basis Mtx, T = powers of t
//
Vec4F Spline::EvaluateBezier ( int b, int k, float u )
{
	Vec4F* keys	= GetElemVec4 ( b, 0 );		// keys
	Vec4F* tans = GetElemVec4 ( b+1, 0 );	// tangents
	...
	float b1 = 3*u*um*um;
	float b2 = 3*u*u*um;
	Vec4F kn1 = (k + 1 >= m_num) ? *(keys + m_num - 1) : *(keys + k + 1);
