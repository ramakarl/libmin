...
void glib::drawText ( Vec2F a, std::string msg, Vec4F clr )
{
	...
	if ( len == 0 ) return;                     // no message to print
  if ( gx.m_font_img.isEmpty() ) return;      // no fonts. need _REQUIRE_IMAGE enabled
...
{		
    ...
    glActiveTexture(GL_TEXTURE0 + 1);
		glBindTexture(GL_TEXTURE_2D, gx.m_white_tex );
...
void glib::selfSetTexture ( int glid ) 
{
		...
		glBindTexture ( GL_TEXTURE_2D, (glid==-1) ? gx.m_white_tex : glid );
		#ifdef EXTRA_CHECKS
...
void gxLib::createShader3D ()
{
	...
	GLchar const * fss =
		...
		"  vec3 R = normalize( vnorm * (2.0f*dot ( vnorm, L)) - L );\n"
		"  if (vnorm.x<-1.f) {\n"
		"     outColor = imgclr;\n"
		"  } else {\n"
		"     vec3 sclr = spec_clr * pow( max(0.0f, dot(R, V)), spec_power ); \n"
		"     vec3 dclr = diff_clr * max( 0.0f, dot ( vnorm, L )); \n"
...
bool gxLib::loadFont ( const char * fontName )
{
  ...
  #ifndef BUILD_IMAGE
    dbgprintf( "  No fonts. Image support not loaded.\n");
    return true;
  #endif
...
void gxLib::drawSet ( int g )
{
	...
	// opengl render setup
		...
		if (sh==S3D) {
    		...
    		// update camera
     		glUniform3f ( gx.mPARAM[sh][SP_CAMPOS], s->cam_from.x, s->cam_from.y, s->cam_from.z);
	...
	if ( sh==S3D ) {
        ...
        checkGL ( "drawSet(3D): glDrawArrays ");
			#endif
		...
		while ( pos < s->size ) {
			...
			// opengl render
				...
				checkGL ( "drawSet(2D): glVertAttrPtr pos 2d");
				glVertexAttribIPointer(slotClr, 1, GL_UNSIGNED_INT,    sizeof(gxVert), (void*) (pos + 12) );
				checkGL ( "drawSet(2D): glVertAttrPtr clr 2d");
